...
export default function AdminAssociates() {
  ...
  const assignedCount = users.filter(u => u.parent_user_id !== null).length;
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-2xl font-serif text-brand-deep-navy">Associates</h2>
          <p className="text-brand-charcoal/60">Manage all Associates across KHU Developers.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
        <div className="bg-white p-6 border border-brand-charcoal/10 rounded-lg shadow-sm">
          <div className="text-xs font-bold tracking-widest uppercase text-brand-charcoal/50 mb-1">Total Associates</div>
          <div className="text-3xl font-serif text-brand-deep-navy">{total}</div>
        </div>
        <div className="bg-white p-6 border border-brand-charcoal/10 rounded-lg shadow-sm">
          <div className="text-xs font-bold tracking-widest uppercase text-brand-charcoal/50 mb-1">Active</div>
          <div className="text-3xl font-serif text-green-700">{activeCount}</div>
        </div>
        <div className="bg-white p-6 border border-brand-charcoal/10 rounded-lg shadow-sm">
          <div className="text-xs font-bold tracking-widest uppercase text-brand-charcoal/50 mb-1">Assigned</div>
          <div className="text-3xl font-serif text-brand-architectural-blue">{assignedCount}</div>
        </div>
        <div className="bg-white p-6 border border-brand-charcoal/10 rounded-lg shadow-sm">
          <div className="text-xs font-bold tracking-widest uppercase text-brand-charcoal/50 mb-1">Unassigned</div>
          <div className="text-3xl font-serif text-brand-gold">{unassignedCount}</div>
        </div>
      </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm whitespace-nowrap">
              <thead className="bg-brand-off-white text-brand-charcoal uppercase text-xs tracking-wider border-b border-brand-soft-grey">
                <tr>
                  <th className="px-6 py-4 font-medium">Associate Code & Name</th>
              ...
              <tbody className="divide-y divide-brand-soft-grey">
                {loading ? (
                  ...
                  <tr><td colSpan={7} className="px-6 py-12 text-center text-brand-charcoal/60">No Associates found.</td></tr>
                ) : (
                  filteredUsers.map(user => (
                    <tr key={user.id} className="hover:bg-brand-warm-white/50 transition-colors group">
                      <td className="px-6 py-4">
                        ...
                        <div className="text-xs text-brand-charcoal/60 mt-1">Associate</div>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4">
                        {user.parent?.parent ? (
                          <div className="text-brand-charcoal hover:underline cursor-pointer">
                            {formatUser(user.parent.parent.user_code, user.parent.parent.full_name)}
                          </div>
                        ) : (
                          <span className="text-brand-charcoal/50 italic">-</span>
                        )}
                      </td>
                      <td className="px-6 py-4">
